...
static void vSenderTask(void* pvParameters)
{
	...
	for (;;)
	{
		...
		// is not specified because the queue should never contain more than one
		// item and therefore never be full.
...
static void vReceiveTask(void* pvParameters)
{
	...
	for (;;)
	{
		...
		{
			vTaskSuspendAll();
			xil_printf("Queue should have been empty!\n");
			xTaskResumeAll();
		}
		...
		// that the task should remain in the Blocked state to wait for data to
		// be available should the queue already be empty. In this case the
		// constant portTICK_RATE_MS is used to convert 300 milliseconds to 
		...
		if (xStatus == pdPASS)
		{
			...
			// received value.
			vTaskSuspendAll();
			xil_printf("Received = %ld\n", lReceivedValue);
			xTaskResumeAll();
		}
		else
		{
			// Data was not received from the queue even after waiting for 100ms.
			// This must be an error as the sending tasks are free running and
			// will be continuously writing to the queue.
			vTaskSuspendAll();
...
int main(void)
{
	...
	if (xQueue != NULL)
	{
		...
		xTaskCreate(vReceiveTask, "Receiver", configMINIMAL_STACK_SIZE,
	...
	// If all is well then main() will never reach here as the schedulder will
